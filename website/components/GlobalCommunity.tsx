...
function WorldMapSVG({ countries }: { countries: CountryData[] }) {
  const maxUsers = Math.max(...countries.map((c) => c.users), 1);
  const mapped = countries
    .map((c) => {
      const coords = COUNTRY_COORDS[c.country];
      if (!coords) return null;
      const x = ((coords[1] + 180) / 360) * 800;
      const y = ((90 - coords[0]) / 180) * 450;
      const r = Math.max(4, Math.min(16, 4 + (c.users / maxUsers) * 12));
      return { ...c, x, y, r };
    })
    .filter(Boolean) as { country: string; users: number; x: number; y: number; r: number }[];

  return (
    <svg viewBox="0 0 800 450" className="w-full h-auto">
      <rect width="800" height="450" fill="#f0f7f2" rx="16" />

      {/* Simplified continent outlines */}
      <ellipse cx="200" cy="180" rx="90" ry="70" fill="#dceade" opacity="0.5" />
      <ellipse cx="200" cy="300" rx="50" ry="70" fill="#dceade" opacity="0.5" />
      <ellipse cx="400" cy="160" rx="110" ry="80" fill="#dceade" opacity="0.5" />
      <ellipse cx="420" cy="300" rx="40" ry="60" fill="#dceade" opacity="0.5" />
      <ellipse cx="520" cy="200" rx="80" ry="60" fill="#dceade" opacity="0.5" />
      <ellipse cx="620" cy="280" rx="60" ry="50" fill="#dceade" opacity="0.5" />
      <ellipse cx="700" cy="340" rx="35" ry="25" fill="#dceade" opacity="0.5" />

      {/* Grid lines */}
      {[0, 100, 200, 300, 400].map((y) => (
        <line key={`h${y}`} x1="0" y1={y} x2="800" y2={y} stroke="#c8dccb" strokeWidth="0.5" opacity="0.4" />
      ))}
      {[0, 160, 320, 480, 640, 800].map((x) => (
        <line key={`v${x}`} x1={x} y1="0" x2={x} y2="450" stroke="#c8dccb" strokeWidth="0.5" opacity="0.4" />
      ))}

      {/* Country pins */}
      {mapped.map((c, i) => (
        <g key={i}>
          <circle cx={c.x} cy={c.y} r={c.r + 3} fill="#4A7C59" opacity="0.15" />
          <circle cx={c.x} cy={c.y} r={c.r} fill="#4A7C59" opacity="0.8" stroke="#2d5a3a" strokeWidth="1.5" />
          {c.r >= 7 && (
            <text x={c.x} y={c.y + 4} textAnchor="middle" fill="#fff" fontSize="9" fontWeight="700">
              {c.users}
            </text>
          )}
        </g>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          <div className="text-center">
            <div className="text-3xl sm:text-4xl font-extrabold text-forest-dark">{data.total_countries}</div>
            <div className="text-sm text-forest-dark/50 mt-1">Countries</div>
          </div>
          <div className="text-center">
            <div className="text-3xl sm:text-4xl font-extrabold text-forest-dark">{data.total_schools}</div>
        ...
        >
          <WorldMapSVG countries={data.countries} />
        </motion.div>
